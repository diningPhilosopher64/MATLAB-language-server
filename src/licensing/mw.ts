...
export async function fetchEntitlements(mhlmUrl: string, accessToken: string, matlabVersion: string): Promise<Entitlement[]> {
    ...
    let entitlementsData = findAllEntitlements(jsonData["describe_entitlements_response"]["entitlements"])
    ...
    let entitlements: Entitlement[] = entitlementsData.map(entitlementData => {
        let entitlement: Entitlement = entitlementData[0]
        return {
