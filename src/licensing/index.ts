...
import { OnlineLicensingError, EntitlementError, LicensingError } from './errors';
import { ExistingLicenseType, FailedMHLMLicensingData, LicensingData, MHLMLicenseType, MHLMLicensingData, NLMLicenseType, NLMLicensingData, ValidCachedLicensingData ,isMHLMLicensingDataType, isNLMLicensingDataType, isExistingLicensingDataType, NoLicensingData, isNoLicensingDataType } from './types';
...
export default class Licensing {
    ...
    // TODO: Is WS_ENV required for this usecase ? As the extension will always run in prod.
    static wsEnv: string = process.env.WS_ENV ?? ""
    static wsEnvSuffix: string = this.wsEnv.includes("integ") ? `-${this.wsEnv}` : "";
    static mwaApiEndpoint: string = `https://login${Licensing.wsEnvSuffix}.mathworks.com/authenticationws/service/v4`;
    static mhlmApiEndpoint: string = `https://licensing${Licensing.wsEnvSuffix}.mathworks.com/mls/service/v1/entitlement/list`
    ...
    static type = {
        // Prefixing with 'MATLAB:' for better clarity on NO_LICENSE and EXISTING_LICENSE types for the status bar icon text.
        NO_LICENSE: 'MATLAB: Not Licensed',
        MHLM_LICENSE: 'Online License Manager',
        NLM_LICENSE: 'Network License Manager',
        EXISTING_LICENSE: 'MATLAB: Existing License'
    }
    ...
    /**
     * Gets the licensing type as a string.
     * @private
     * @returns {string} The licensing type.
     */
    private getLicensingType(): string {
        if (this.isNLMLicensing()) {
            ...
            return Licensing.type.NO_LICENSE;
        }
    }
    ...
    private async setLicensingToMHLM(licenseData: any) {
        ...
        try {
            ...
            if (error instanceof OnlineLicensingError || error instanceof EntitlementError) {
                ...
                this.data = null as NoLicensingData
            }
    ...
    async updateEntitlements(): Promise<boolean> {
        ...
        let mhlmData = this.data as MHLMLicensingData;
