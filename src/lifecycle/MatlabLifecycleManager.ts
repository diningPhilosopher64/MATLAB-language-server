...
export interface MatlabLifecycleEvent {
    ...
    async launchMatlab (): Promise<void> {
        ...
        return await new Promise<void>(async resolve => {
            ...
            watcher.on('add', async () => {
                ...
                this._matlabPid = connectionInfo.pid
                this._matlabRelease = connectionInfo.release
                const { port, certFile, sessionKey } = connectionInfo

                this._matlabConnection?.initialize(port, certFile).then(() => {
                    LifecycleNotificationHelper.notifyConnectionStatusChange(ConnectionState.CONNECTED)
                    Logger.log(`Connected to MATLAB ${connectionInfo.release}`)
                    reportTelemetryAction(Actions.StartMatlab, connectionInfo.release)
            ...
            this._launchMatlabProcess(outFile)
        })
    }
